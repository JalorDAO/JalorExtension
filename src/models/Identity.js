...
import { BlockchainsArray, getChainType, chainTypes } from '../models/Blockchains'
...
export default class Identity {
    ...
    constructor(){
        ...
        // Requireable fields, change to only one account
        this.accounts = {};
    ...
    setAccount(network, account){ 
        // only related one account
        if(Object.keys(this.accounts).length > 0){
            throw new Error('identity can only bind one account')
        }
        this.accounts[network.unique()] = account;
    }
    getAccount() {
        let account
        for (const i in this.accounts) {
            account = this.accounts[i]
            // account.blockchain = this.accounts[i].blockchain()
            break;
        }
        return account
    }
    ...
    getName() {
        ...
        const { name, authority,address } = account
        const chainType = this.getBlockchainType()
        switch(chainType){
    ...
    getInfo(){
        const account = this.getAccount()
        const { name, address, publicKey } = account
        const chainType = this.getBlockchainType()
        switch(chainType){
            case chainTypes.ETHEREUM:
                return address;
    ...
    getBlockchainType() {
        return getChainType(this.getAccount().blockchain())
    }
