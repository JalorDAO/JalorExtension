    ...
    <section class="networks">
        ...
        <section class="p20 scroller with-search" v-if="keypairs.length">
            <section v-for="keypair in filterBySearch()" class="panel-box">
                ...
                <!-- Header -->
                <section class="panel">
                    <figure class="header big"><i class="fa fa-key"></i>{{keypair.name}}</figure>
                    <figure class="header small margin"><i class="fa fa-key"></i>{{keypair.publicKey.substr(0,12)}}...</figure>
                    <figure class="header small margin"><i class="fa fa-chain"></i>{{keypair.blockchain.toUpperCase()}}</figure>
                </section>

                <!-- Actions -->
                <section class="panel">
                    <section class="actions">
                        <figure class="action blue" v-on:click="copyPublicKey(keypair)"><i class="fa fa-copy"></i></figure>
                        <figure class="action blue" v-on:click="exportPrivateKey(keypair)"><i class="fa fa-database"></i></figure>
                        <figure class="action red right" v-on:click="deleteKeypair(keypair)"><i class="fa fa-trash"></i></figure>
                    </section>
                </section>
    ...
    export default {
        ...
        methods: {
            ...
            exportPrivateKey(keypair){
                this.$router.push({ name:RouteNames.EXPORT,query:{unique:keypair.unique()} })            
            },
                    ...
                    if(usedInIdentities.length){
                        usedInIdentities.map(_id => {
                            ...
                            Object.keys(id.accounts).map(network => {
                                ...
                                && id.accounts[network].keypairUnique === keypair.unique())
                                    // id.removeAccount(network);
                                    scatter.keychain.removeIdentity(id.hash)
                            });
