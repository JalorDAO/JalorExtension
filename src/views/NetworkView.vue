<template>
    <section class="network scroller" v-if="network">
        ...
        <section class="panel">
            ...
            <sel :disabled="isEdit" :selected="initSelectBlockchain" :options="blockchains" :parser="blockchain => blockchain.key" v-on:changed="changed => bind(changed.value, 'blockchain')"></sel>
            <cin :disabled="isEdit" :placeholder="locale(langKeys.PLACEHOLDER_Name)" :text="network.name" v-on:changed="changed => bind(changed, 'name')"></cin>
            ...
            <cin :disabled="isEdit" :placeholder="locale(langKeys.GENERIC_ChainID)" :text="network.chainId" v-on:changed="changed => bind(changed, 'chainId')"></cin>
        </section>
...
<script>
    ...
    export default {
        data(){ return {
            ...
            network:null,
            isEdit:false,
            initSelectBlockchain:BlockchainsArray[0]
        }},
        computed: {
            ...mapState([
                'scatter'
        ...
        mounted(){
            ...
            if(network){
                this.network = Network.fromJson(JSON.parse(network));
                this.isEdit = true;
                const chain = this.network.blockchain
                this.initSelectBlockchain = this.blockchains.find(blockchain=>blockchain.value==chain)
            } else {
        ...
        methods: {
            ...
            async saveNetwork(){
                ...
                if(this.isEdit){
                    scatter.settings.networks = scatter.settings.networks.map(network=>{
                        if(network.unique() == this.network.unique()){
                            return this.network
                        } else {
                            return network
                        }
