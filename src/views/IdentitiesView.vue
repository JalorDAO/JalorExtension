    ...
    <section class="identities">
        ...
        <section v-if="!identities.length" class="nothing-here">
            ...
            <figure class="sub-header">
                ...
                <br><br>
                <btn :text="locale(langKeys.BUTTON_AssociateAccount)" v-on:clicked="createIdentity" margined="true"></btn>
            </figure>
        ...
        <section v-if="identities.length" class="p20 scroller with-search">
            <section v-for="identity in filterBySearch()" class="panel-box">
                ...
                <!-- Header -->
                <section class="panel">
                    <figure class="header big"><i class="fa fa-user"></i>{{identity.getName()}}</figure>
                    <figure class="header small"><i class="fa fa-chain"></i>{{identity.getBlockchain().toUpperCase()}}</figure>
                </section>
                ...
                <section class="panel">
                    <section class="actions">
                        ...
                        <section v-if="Object.keys(identity.accounts).length">
                            ...
                            -->
                        <figure class="action blue" v-on:click="copyAddress(identity)"><i class="fa fa-copy"></i></figure>
                        <figure class="action blue" v-on:click="showQrcode(identity)"><i class="fa fa-qrcode"></i></figure>
                        <figure class="action red right" v-on:click="removeIdentity(identity)"><i class="fa fa-trash"></i></figure>
                    </section>
                </section>
    ...
    export default {
        ...
        methods: {
            ...
            copyAddress(identity){
                ...
                copier.value = identity.getAddress();
                copier.select();
                document.execCommand("copy");
            },
            showQrcode(identity){
                this.$router.push({ name:RouteNames.QRCODE, query: { qrcode: identity.getAddress() } })
            },
