...
import Goc from 'eosjs-classic'
let { ecc, Fcbuffer } = Goc.modules;
...
export default class GOC extends Plugin {

    constructor() { super(Blockchains.GOC, PluginTypes.EOSIO) }
    ...
    async getEndorsedNetwork() {
        return new Promise((resolve, reject) => {
            resolve(new Network(
                ...
                443,
                Blockchains.GOC,
                '4abcef3ea73a7cd42b2e39e247355048b684225d21d68aa303dd22712e8ca05d'
            ));
        });
    }
    ...
    async isEndorsedNetwork(network) {
        ...
        return network.unique() === endorsedNetwork.unique();
    }
    ...
    importAccount(keypair, network, context, accountSelected) {
        const getAccountsFromPublicKey = (publicKey, network) => {
            return new Promise((resolve, reject) => {
                const goc = Goc({ httpEndpoint: `${network.protocol}://${network.hostport()}` });
                //console.log(`send to ${network.protocol}://${network.hostport()}`);
                goc.getKeyAccounts(publicKey).then(res => {
                    if (!res || !res.hasOwnProperty('account_names')) { resolve([]); return false; }

                    Promise.all(res.account_names.map(name => goc.getAccount(name).catch(e => resolve([])))).then(multires => {
                        let accounts = [];
    ...
    // 指定账户导入
    inputAccount(accountName, keypair, network, context, accountSelected) {
        const getAccountsFromName = (name, publicKey, network) => {
            return new Promise((resolve, reject) => {
                const goc = Goc({ httpEndpoint: `${network.protocol}://${network.hostport()}` });
                goc.getAccount(name)
                    .then(account => {
        ...
        getAccountsFromName(accountName, keypair.publicKey, network).then(accounts => {
            switch (accounts.length) {
                case 0: context[Actions.PUSH_ALERT](AlertMsg.NoAccountsFound()); reject(); return false;
                // Only one account, so returning it
                case 1: accountSelected(Account.fromJson({ name: accounts[0].name, authority: accounts[0].authority, publicKey: keypair.publicKey, keypairUnique: keypair.unique() })); break;
    ...
    privateToPublic(privateKey) { return ecc.privateToPublic(privateKey, "GOC"); }
    validPrivateKey(privateKey) { return ecc.isValidPrivate(privateKey); }
    validPublicKey(publicKey) { return ecc.isValidPublic(publicKey, "GOC"); }
    randomPrivateKey() { return ecc.randomKey(); }
    async mnemonicToPrivate(MnemonicStr) {
        if (!bip39.validateMnemonic(MnemonicStr)) {
            return new Error('Mnemonic format error')
        }
        const seed = bip39.mnemonicToSeedHex(MnemonicStr)
    ...
    async getBalances(account, network, code = 'gocio.token', table = 'accounts') {
        const goc = Goc({ httpEndpoint: `${network.protocol}://${network.hostport()}`, chainId: network.chainId });
        const contract = await goc.contract(code);
        return await goc.getTableRows({
            json: true,
            code,
            scope: account.name,
            table,
    ...
    signatureProvider(...args) {
        ...
        return (network, _goc, _options = {}, protocol = 'http') => {
            ...
            return proxy(_goc({ httpEndpoint, chainId: _options.chainId }), {
                get(gocInstance, method) {
                    ...
                    return (...args) => {
                        ...
                        const signProvider = async signargs => {
                            ...
                            // Friendly formatting
                            signargs.messages = await requestParser(_goc, signargs, httpEndpoint, args[0], _options.chainId);
                        ...
                        return new Promise((resolve, reject) => {
                            _goc(Object.assign(_options, { httpEndpoint, signProvider }))[method](...args)
                                .then(result => {
...
const requestParser = async (_goc, signargs, httpEndpoint, possibleSigner, chainId) => {

    const goc = _goc({ httpEndpoint, chainId });
    ...
    await Promise.all(contracts.map(async contractAccount => {
        ...
        if (cachedABI === 'object' && cachedABI.timestamp > +new Date((await goc.getAccount(contractAccount)).last_code_update))
            abis[contractAccount] = goc.fc.abiCache.abi(contractAccount, cachedABI.abi);

        else {
            abis[contractAccount] = (await goc.contract(contractAccount)).fc;
