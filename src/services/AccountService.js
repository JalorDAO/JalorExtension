...
export default class AccountService {
     ...
     * @returns {Promise}
     */
    static importFromAccountAndKey(name, keypair, network, context) {
        return new Promise((resolve, reject) => {
            const accountSelected = (account) => resolve({ keypair, account });

            // Accounts for this blockchain need importation
            if (PluginRepository.plugin(network.blockchain).accountsAreImported())
                PluginRepository.plugin(network.blockchain).inputAccount(name, keypair, network, context, accountSelected);
