...
export default {
    ...
    [KEYS.GENERIC_Identity]:`导入账户`,
    [KEYS.GENERIC_Scan_Identity]:`搜索账户`,
    [KEYS.GENERIC_Input_Identity]:`输入账户`,
    ...
    [KEYS.GENERIC_Import_key]:'导入密钥',
    [KEYS.GENERIC_No_key]:'还未导入密钥？',
    [KEYS.GENERIC_ChainID]:'链 ID',
    [KEYS.GENERIC_Blockchain]:'区块链',
    [KEYS.GENERIC_Account]:'帐户',
    [KEYS.GENERIC_Confirm]:'确认',
    [KEYS.GENERIC_Done]:'完成',
    [KEYS.GENERIC_Type]:'类型',
    ...
    [KEYS.BUTTON_AssociateAccount]:'关联账户',
    ...
    [KEYS.BUTTON_DestroyScatter]:'删除 Jalor',
    [KEYS.BUTTON_CreateNewScatter]:'创建新的 Jalor',
    [KEYS.BUTTON_LoadFromBackup]:'从备份中恢复',
    [KEYS.BUTTON_Unlock]:'解锁',
    [KEYS.BUTTON_ExportScatter]:'导出 Jalor',
    [KEYS.BUTTON_ExportPrivate]:'备份密钥',
    [KEYS.BUTTON_SkipExportPrivate]:'跳过备份',
    ...
    [KEYS.BUTTON_WroteDownMnemonic]:'我已经抄写',



    [KEYS.MAINMENU_Identities]:'账户地址',
    [KEYS.MAINMENU_Keys]:'密钥对',
    [KEYS.MAINMENU_Permissions]:'权限',
    [KEYS.MAINMENU_History]:'事件',
    [KEYS.MAINMENU_Lock]:'锁定',
    [KEYS.MAINMENU_Settings]:'设置',


    [KEYS.IDENTITIES_Header]:`你目前尚未关联账号。`,
    [KEYS.IDENTITIES_Description]:`与您的密钥对相关联的区块链地址或账户`,
...
     `,
    ...
    `,
    [KEYS.PERMISSION_RevokeIdentity]:`撤销权限`,
    ...
    [KEYS.HISTORIES_Description]:`
        在此您能看到所有 Jalor 的事件列表.
        但在您的 scatter 外的帐户上发生的事件不会显示.
     `,
    [KEYS.HISTORIES_Note]:`
        注意：当从设置面板中的备份选项中导出您的 Jalor 数据时，不会备份这些事件。
        所以当您再次导入时，您的历史记录是空的。如果你想导出这些历史事件，
        将来您可以使用此面板中导航栏上的操作按钮来完成此操作。
    `,

    [KEYS.SETTINGSMENU_Networks]:'网络',
    [KEYS.SETTINGSMENU_Network]:'网络',
    [KEYS.SETTINGSMENU_Keypairs]:'密钥对',
    [KEYS.SETTINGSMENU_Qrcode]:'二维码',
    [KEYS.SETTINGSMENU_ExportPrivate]:'备份密钥',
    ...
    [KEYS.BACKUP_Description]:`
        导出您的 Jalor 后，您将可以将其导入其他设备。导出文件将被加密，
        所以请注意备份你的助记符和密码。否则将不能导入。
    `,
    [KEYS.BACKUP_Key_Description]:`
        请勿使用网络传输您的密钥，最好抄写备份您的密钥，
        截图和复制都有未知的风险，如果是设备间传输可以使用扫描二维码的方式。
    `,
    [KEYS.IMPORT_Header]:`导入加密的备份`,
    [KEYS.IMPORT_Description]:`
        导入你的加密钥匙串文件将重建您的 Jalor 钥匙串，但是它不会导入您原有的网络
        或帐户设置
    `,
    ...
    [KEYS.DESTROY_Header]:`销毁 Jalor `,
    [KEYS.DESTROY_Description]:`
        您即将销毁您的整个 Jalor 钥匙串。恢复 Jalor 的唯一方法是
        导入以前导出的 Jalor JSON 文件。 否则你将没有办法再使用你的身份。
        请确保在着手这个操作前，已经从备份设置面板中导出了您的 Jalor 。
    `,
    ...
    [KEYS.KEYPAIR_Description]:`
        您可以通过此界面来生成密钥对。密钥对是在你本地机器上生成并且不会被传送到其它任何地方。
    ...
    [KEYS.PROMPT_RemovingIdentity]:name => [
        ...
        `您即将删除名称为 '${name}' 的帐户，并且所有的权限也将删除，删除帐户不可逆。`
    ],
    //2
...
         此事务的某些属性的更改，并且仅当其他属性发生更改时，
    ...
    [KEYS.REQUEST_UpdateVersion]:[
        ...
        `注意，我们让应用能够检查 Jalor 的版本，但它们有可能让你下载恶意程序，请确保从正确的地址下载 Jalor .`
    ],
    [KEYS.REQUEST_Welcome]:[
        `欢迎使用 Jalor!`,
        `Jalor团队对您的数字资产不承担任何责任，我们已采取一切措施来确保Jalor的使用安全。您有责任不断确保自己的计算机没有恶意软件。`,
        `如您愿意使用Jalor，您将放弃对Jalor团队采取法律行动的权力，并承认您自己是Jalor所存储密钥和数据的唯一合法所有者。`
    ]
}
